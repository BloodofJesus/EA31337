...
#include <EA31337-classes\Errors.mqh>
...
#include <EA31337-classes\Timeframe.mqh>
...
bool UpdateIndicator(int type = EMPTY, ENUM_TIMEFRAMES tf = PERIOD_M1, string symbol = NULL) {
  bool success = true;
  static datetime processed[FINAL_ENUM_INDICATOR_INDEX][FINAL_ENUM_TIMEFRAMES_INDEX];
...
bool CloseOrder(int ticket_no = EMPTY, int reason_id = EMPTY, bool retry = true) {
  ...
  if (result) {
    ...
    last_debug = Msg::ShowText(Order::OrderTypeToString(), "Debug", __FUNCTION__, __LINE__, VerboseDebug);
    #ifdef __advanced__
    ...
    #else
      if (VerboseDebug) Print(__FUNCTION__, ": Closed order " + IntegerToString(ticket_no) + " with profit " + DoubleToStr(Order::GetOrderProfit()) + " pips; " + Order::OrderTypeToString());
    #endif
  } else {
    err_code = GetLastError();
...
int CloseOrdersByType(ENUM_ORDER_TYPE cmd, int strategy_id, int reason_id, bool only_profitable = false) {
   ...
   for (order = 0; order < OrdersTotal(); order++) {
      if (OrderSelect(order, SELECT_BY_POS, MODE_TRADES)) {
        ...
        if (VerboseDebug)
          Print(__FUNCTION__ + "(" + Order::OrderTypeToString(cmd) + ", " + IntegerToString(strategy_id) + "): Error: Order: " + IntegerToString(order) + "; Message: ", Errors::GetErrorText(err_code));
...
bool UpdateStats() {
  ...
  PrintFormat("Orders: %d/%d (OP_BUY:%d/OP_SELL:%d), Balance: %g (Eq: %g), Total SL: %g (B:%g/S:%g), Total TP: %g (B:%g/S:%g), Calc Money: SL:%g/TP:%g",
    GetTotalOrders(), OrdersTotal(), Orders::GetOrdersByType(OP_BUY), Orders::GetOrdersByType(OP_SELL),
...
bool CheckMarketCondition1(int cmd, ENUM_TIMEFRAMES tf = PERIOD_M30, int condition = 0, bool default_value = true) {
  ...
  if ((condition &   2) != 0) result &= UpdateIndicator(S_SAR, tf)       && ((cmd == OP_BUY && sar[period][CURR] < Open[0]) || (cmd == OP_SELL && sar[period][CURR] > Open[0]));
  if ((condition &   4) != 0) result &= UpdateIndicator(S_RSI, tf)       && ((cmd == OP_BUY && rsi[period][CURR] < 50) || (cmd == OP_SELL && rsi[period][CURR] > 50));
  if ((condition &   8) != 0) result &= MA::Update(tf)        && ((cmd == OP_BUY && Ask > ma_slow[period][CURR]) || (cmd == OP_SELL && Ask < ma_slow[period][CURR]));
//if ((condition &   8) != 0) result &= MA::Update(tf)        && ((cmd == OP_BUY && ma_slow[period][CURR] > ma_slow[period][PREV]) || (cmd == OP_SELL && ma_slow[period][CURR] < ma_slow[period][PREV]));
  if ((condition &  16) != 0) result &= ((cmd == OP_BUY && Ask < Open[CURR]) || (cmd == OP_SELL && Ask > Open[CURR]));
  if ((condition &  32) != 0) result &= UpdateIndicator(S_BANDS, tf)     && ((cmd == OP_BUY && Open[CURR] < bands[period][CURR][BANDS_BASE]) || (cmd == OP_SELL && Open[CURR] > bands[period][CURR][BANDS_BASE]));
  if ((condition &  64) != 0) result &= UpdateIndicator(S_ENVELOPES, tf) && ((cmd == OP_BUY && Open[CURR] < envelopes[period][CURR][MODE_MAIN]) || (cmd == OP_SELL && Open[CURR] > envelopes[period][CURR][MODE_MAIN]));
  if ((condition & 128) != 0) result &= UpdateIndicator(S_DEMARKER, tf)  && ((cmd == OP_BUY && demarker[period][CURR] < 0.5) || (cmd == OP_SELL && demarker[period][CURR] > 0.5));
  if ((condition & 256) != 0) result &= UpdateIndicator(S_WPR, tf)       && ((cmd == OP_BUY && wpr[period][CURR] > 50) || (cmd == OP_SELL && wpr[period][CURR] < 50));
 ...
 */
int GetCmdByOrders() {
  int buys = Orders::GetOrdersByType(OP_BUY);
  int sells = Orders::GetOrdersByType(OP_SELL);
  if (buys > sells) return OP_BUY;
  if (sells > buys) return OP_SELL;
  return EMPTY;
}
 ...
 */
bool InitClasses() {
  account = new Account(V_INFO);
  logger = new Log(V_INFO);
  market = new Market(_Symbol);
  summary_report = new SummaryReport(init_balance);
  ticks = new Ticks();
  timeframe = new Timeframe(PERIOD_CURRENT, _Symbol);
...
bool AccCondition(int condition = C_ACC_NONE) {
  switch(condition) {
    ...
    case C_ACC_IN_TREND:
      last_cname = "InTrend";
      return (Convert::ValueToOp(curr_trend) == OP_BUY && Orders::GetOrdersByType(OP_BUY)  > Orders::GetOrdersByType(OP_SELL))
          || (Convert::ValueToOp(curr_trend) == OP_SELL && Orders::GetOrdersByType(OP_SELL) > Orders::GetOrdersByType(OP_BUY));
...
string DisplayInfoOnChart(bool on_chart = true, string sep = "\n") {
  ...
  output = indent + "------------------------------------------------" + sep
                  ...
                  + indent + (RiskMarginTotal >= 0 ? StringFormat("| Risk margin level: Total: %g (Buy:%g, Sell:%g)", ea_margin_risk_level[2], ea_margin_risk_level[OP_BUY], ea_margin_risk_level[OP_SELL]) : "| Risk margin level: Disabled") + sep
...
string GetOrdersStats(string sep = "\n") {
  ...
  total_orders_text += StringFormat(" +%d/-%d", Orders::GetOrdersByType(OP_BUY),  Orders::GetOrdersByType(OP_SELL));
...
string GetAccountTextDetails(string sep = "; ") {
   return StringConcatenate("Account Details: ",
      ...
      "No of Orders: ", total_orders, " (BUY/SELL: ", Orders::GetOrdersByType(OP_BUY), "/", Orders::GetOrdersByType(OP_SELL), ")", sep,
...
bool ActionExecute(int aid, int id = EMPTY) {
  ...
  switch (aid) {
    ...
    case A_CLOSE_ALL_LOSS_SIDE: /* 7 */
      cmd = GetProfitableSide();
      result = cmd != EMPTY ? ActionCloseAllOrdersByType(Order::NegateOrderType(cmd), reason_id) : true;
      break;
...
bool OpenOrderCondition(int cmd, int sid, datetime time, int method) {
  ...
  if ((method & 128) != 0) result &= MA::Update(tf) && Trade_MA(cmd, tf, 0, 0);
  return (result);
}
